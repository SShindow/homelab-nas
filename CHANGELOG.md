...
- **Cleanuparr** auto-cleans stalled/failed/malicious downloads out of qBittorrent and tells Sonarr/Radarr/Lidarr to re-search — added ahead of more people generating download traffic than one person's own usage patterns
...
## 2026 — Media Automation: the arr-stack (Prowlarr, qBittorrent, Sonarr, Radarr, Bazarr, Seerr) + Lidarr
- Built a request-to-playback pipeline: Seerr (requests) → Prowlarr (indexer aggregation) → Sonarr/Radarr/Lidarr (matching/management) → qBittorrent (download) → Bazarr (subtitles) → Jellyfin (playback), deployed as a single TrueNAS SCALE Custom App (`arr-stack`) on a shared Docker bridge network (`arrs-network`)
- Hit the classic `localhost`-vs-container-name bug on every single app-to-app pairing in the stack (Prowlarr↔Sonarr/Radarr/Lidarr, Sonarr/Radarr↔qBittorrent) — fixed each by swapping in the target's container name
...
- Fixed qBittorrent's WebUI returning a bare `Unauthorized`: recent qBittorrent versions added Host Header Validation, which rejects any request not arriving via `localhost` or a whitelisted domain — this NAS is reached via Tailscale IP, so `WebUI\HostHeaderValidation=false` had to be set directly in `qBittorrent.conf` (no environment variable exposes it)
- Worked through three rounds of qBittorrent save-path typos before landing on the real structure (`downloads/torrents` for completed, `downloads/incomplete` for in-progress), then found category-specific save paths were silently ignored until the easy-to-miss `Use Category paths in Manual Mode` setting was enabled
- Found Radarr's default `HD-1080p` quality profile silently included Remux-1080p (an essentially uncompressed Blu-ray rip) as a top-ranked tier, defeating the point of choosing 1080p over 4K for storage reasons — unchecked it; Sonarr's equivalent profile was already clean
...
- Hit VNPT (the Vietnamese ISP)'s DNS interference a second and third time, on Prowlarr's Nyaa.si indexer and on Seerr's entire TMDB-backed discover/search — same fix as before, a per-container `dns: [1.1.1.1, 1.0.0.1]` override
- Adopted deliberate indexer redundancy (8 indexers across movies/TV/anime categories) after enough individual public-torrent sites proved unreliable for reasons unrelated to this NAS; 1337x remains Cloudflare-blocked and not worth a FlareSolverr deployment given coverage is already met
