...
- **Real-world throughput finding, discovered from the first real movie streamed through the pipeline rather than during setup:** a generic speedtest from the NAS shows a healthy 161 Mbps, but the actual Vietnam↔Germany path measured only ~3.78 Mbps at the time — see the Jellyfin entries below for the full diagnosis, including a 2026-09-08 follow-up measurement that found this varies a lot. TV-episode bitrates stream live from Germany fine regardless; movie-tier bitrates need a capped stream or a local download first as the safe default
...
- **2026-08-28 — Real-world remote playback throughput, root-caused.** A stream failing on a heavier release (Blu-ray-tier bitrate, Atmos audio) led to an `iperf3` test run directly over the Tailscale link, ruling out NAS CPU (74.7% idle), a DERP-relayed connection (confirmed genuinely peer-to-peer via `tailscale status`), and raw bandwidth (a generic speedtest showed a healthy 161.83 Mbps) in that order. The measured cause: sustained throughput of only ~3.78 Mbps over the actual Vietnam↔Germany route at that moment, with heavy retransmissions and a collapsing TCP congestion window — real international packet loss on the day, not a config problem. A seedbox-relay alternative was researched and declined; the practical workaround adopted is a manually bitrate-capped stream, or downloading fully before playback, for anything above TV-episode bitrates.
- **2026-09-08 — Same `iperf3` test repeated, very different result.** Running the identical command between the identical two endpoints three weeks later measured **~22.6 Mbps sustained — nearly 6× the first result**, with no configuration change on either end. Revised the conclusion: this route's throughput is genuinely variable day to day, not a fixed ~3.78 Mbps ceiling — the bitrate-cap workaround stays in place as the safe default since there's no way to know in advance which regime a given session will land in. Full detail (both measurements side by side) in [Media Streaming](docs/media-jellyfin.md#real-world-remote-playback-throughput--variable-not-fixed-2026-08-28-updated-2026-09-08); the reproduction steps are now documented in [Troubleshooting](docs/troubleshooting.md#8-a-generic-speedtest-measures-the-wrong-thing-for-cross-country-remote-access)
